        ...
        self.tag("and-left")(max, n, rng) match
          case Passed(a, n) =>
            that.tag("and-right")(max, n, rng) match
              case Passed(s, m) =>
                Passed(s, TestCases.fromInt(n.toInt + m.toInt))
              case x => x
          case x => x
...
object Gen:
  ...
  def listOfN[A](n: Int, g: Gen[A]): Gen[List[A]] =
    Gen(
      ...
      cartesian(LazyList.continually(g.exhaustive).take(n)).map(l =>
  ...
  def cartesian[A](s: LazyList[LazyList[A]]): LazyList[LazyList[A]] =
    s.foldRight(LazyList(LazyList[A]()))((hs, ts) =>
      map2LazyList(hs, ts)(LazyList.cons(_, _))
    )
   ...
   */
  def map2LazyList[A, B, C](sa: LazyList[A], sb: => LazyList[B])(
      f: (A, => B) => C
  ): LazyList[C] =
    for
      a <- sa
  ...
  def interleave[A](s1: LazyList[A], s2: LazyList[A]): LazyList[A] =
    s1.map(Some(_))
      ...
      .flatMap((a, a2) => LazyList((a.toList ++ a2.toList)*))
  ...
  lazy val pint2: Gen[Par[Int]] = choose(-100, 100)
    .listOfN(choose(0, 20))
    .map(l =>
      l.foldLeft(Par.unit(0))((p, i) => Par.fork { p.map2(Par.unit(i))(_ + _) })
    )
